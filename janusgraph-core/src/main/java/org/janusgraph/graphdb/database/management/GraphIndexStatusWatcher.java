    ...
    public GraphIndexStatusReport call() throws InterruptedException {
        ...
        while (true) {
            ...
            if (!notConverged.isEmpty()) {
                LOGGER.info("Some key(s) on index {} do not currently have status(es) {}: {}", graphIndexName, statusesToString(), waitingOn);
            } else {
                LOGGER.info("All {} key(s) on index {} have status(es) {}", converged.size(), graphIndexName, statusesToString());
            ...
            if (timedOut) {
                LOGGER.info("Timed out ({}) while waiting for index {} to converge on status(es) {}",
                        timeout, graphIndexName, statusesToString());
